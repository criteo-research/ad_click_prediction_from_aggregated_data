...
def followGradWithLinesearch(
    ...
    nbiter=100,
    alpha=0.1,
    usePrecondAtOptim=True,
    usePrecondInDescent=False,
    endIterCallback=None,
):
    basemodel = model
    if usePrecondAtOptim:
        h = model.computeInvHessianDiagAtOptimum()
        model = WrapWithPrecond(model, model.computeInvHessianDiagAtOptimum())
    else:
        h = np.ones(len(model.parameters))

    def computeGrad(x):
        return model.computeGradientAt(x)
    ...
    for i in range(0, nbiter):
        g = computeGrad(x)
        d = -g
        if usePrecondInDescent:
            d *= basemodel.computeInvHessianDiag() / h
        x, g = lineSearch.Search(x, d, g)
        lineSeatchMsg = lineSearch.getInfoStr()

        if endIterCallback is not None:
            endIterCallback()

        llh = model.computeLoss()
